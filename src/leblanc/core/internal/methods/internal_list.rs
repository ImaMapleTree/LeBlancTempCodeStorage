...
pub fn _internal_list_append_(_self: Arc<Mutex<LeBlancObject>>, arguments: &mut [Arc<Mutex<LeBlancObject>>]) -> Arc<Mutex<LeBlancObject>> {
...
pub fn _internal_list_iterate_(_self: Arc<Mutex<LeBlancObject>>, _arguments: &mut [Arc<Mutex<LeBlancObject>>]) -> Arc<Mutex<LeBlancObject>> {
    let mut borrowed = _self.lock().unwrap();
    let list: &mut LeblancList = borrowed.data.mut_data().unwrap();
    leblanc_object_iterator(Box::new(LeblancVecIterator::new(list.internal_vec.clone()))).to_mutex()
}
pub fn _internal_list_length_(_self: Arc<Mutex<LeBlancObject>>, _arguments: &mut [Arc<Mutex<LeBlancObject>>]) -> Arc<Mutex<LeBlancObject>> {
    let mut borrowed = _self.lock().unwrap();
    let list: &mut LeblancList = borrowed.data.mut_data().unwrap();
