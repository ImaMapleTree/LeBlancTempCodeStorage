...
pub fn _internal_iterator_filter_(_self: LBObject, _arguments: Vec<LBObject>) -> LBObject {
    ...
    match iterator.transformed() {
        Some(trans_iter) => {
            trans_iter.filter(_arguments[0].data.get_inner_method().unwrap().leblanc_handle.clone());
            drop(borrowed);
            _self.clone()
        },
        None => {
            let mut new_iter = TransformedIterator::new(iterator.iterator.clone());
            new_iter.filter(_arguments[0].data.get_inner_method().unwrap().leblanc_handle.clone());
            leblanc_object_iterator(Box::new(new_iter))
        }
    }
}
...
pub fn _internal_iterator_map_(_self: LBObject, _arguments: Vec<LBObject>) -> LBObject {
    ...
    match iterator.transformed() {
        Some(trans_iter) => {
            trans_iter.map(_arguments[0].data.get_inner_method().unwrap().leblanc_handle.clone());
            drop(borrowed);
            _self
        },
        None => {
            let mut new_iter = TransformedIterator::new(iterator.iterator.clone());
            new_iter.map(_arguments[0].data.get_inner_method().unwrap().leblanc_handle.clone());
            leblanc_object_iterator(Box::new(new_iter))
        }
    }
}