...
use crate::Heap;
use crate::lbvm::LBVM;
use crate::leblanc::unsafe_raw_vec::UnsafeRawVec;
...
impl<T, A: Allocator> UnsafeVec<T, A> {
    ...
    pub fn push_quick(&mut self, value: T) {
        ...
        unsafe {
            *self.buf.ptr.as_ptr().add(self.len) = value;
            self.len += 1;
        }
    }
    ...
    pub fn split_off_as_vec(&mut self, at: usize, capacity: usize) -> Vec<T, A>
        where
            A: Clone
    {

        if at == 0 {
            let n = unsafe { Vec::from_raw_parts_in(self.buf.ptr.as_ptr(), self.len, capacity, self.allocator().clone()) };
            self.buf.cap = 0;
            self.buf.ptr = Unique::dangling();
            self.len = 0;
            //println!("New self: {:?}", self);
            return n
            /*let unsafe_vec =  mem::replace(
                self,
        ...
        let other_len = self.len - at;
        let mut other = Vec::with_capacity_in(capacity, self.allocator().clone());
        //let other_ptr = unsafe { alloc(Layout::array::<T>(capacity).unwrap()) } as *mut T;
        //let mut other = unsafe { Vec::from_raw_parts_in(other_ptr, other_len, capacity, self.allocator().clone()) };
        ...
        unsafe {
            ...
            ptr::copy_nonoverlapping(self.buf.ptr.as_ptr().add(at), other.as_mut_ptr(), other.len());
        }
        other
    }
    ...
    #[inline(always)]
    pub unsafe fn split_off_bounded_as_vec(&mut self, at: usize) -> Vec<T, A>
        where
            A: Clone,
    {
